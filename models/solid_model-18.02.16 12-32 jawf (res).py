...
def __create_workpiece_partion():
    p = mdb.models['Model-1'].parts['Part']
    p.PartitionCellByPlaneThreePoints(point1=(0,0,0), point2=(0,0,1), point3=(0,1,1), cells=p.cells)
...
def fix_part_face():
    ...
    # faces1 = f1.getSequenceFromMask(mask=('[#204 ]', ), )
    x1,y1 = (inner+(outer-inner)/2,0)
    x2,y2 = (-(inner+(outer-inner)/2),0)
    x3,y3 = (0,inner+(outer-inner)/2)
    x4,y4 = (0,-(inner+(outer-inner)/2))
    side1Faces1 = s1.findAt(((x1, y1, 0), ), 
                            ((x2, y2, 0), ),
                            ((x3, y3, 0), ),
                            ((x4, y4, 0), ))
    
    region = a.Set(faces=side1Faces1, name='PART-FACE-FIX-SET')
    mdb.models['Model-1'].DisplacementBC(name='PART-FACE-FIX', createStepName='Step-2', 
...
if __name__ == "__main__":
    ...
    parser.add_argument("-jawf", nargs=1)
