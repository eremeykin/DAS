...
public abstract class AbstractModelParameterDao implements ModelParameterDao {
    ...
    protected final ModelParameter buildModelParameter(ParameterEntry entry) throws DaoException {
        ...
        String value = entry.get(ParameterEntry.Field.VALUE);
        ...
        try {
            ...
            ModelParameter parameter = new ModelParameter(id, name, value, argScript, comment, master, properties);
            return parameter;
...
//        ValueDao valueDao = new DbValueDao(source);
        ...
        List<Value> allSuitable = new ArrayList<>();
        if (table != null && column != null) {
            allSuitable = valueDao.getAllSuitable(table, column, DaoConstants.MATERIALS_KEY);
        }
